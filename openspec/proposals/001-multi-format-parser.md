...
经仲裁（见 `澄清1.md` + `designs/001`）：**以类型化文档树为规范 IR**，兼容 span（可编辑 PDF）、AST（docx）与结构化 markdown（OCR 服务，D11）。现需新建 `document2chunk` 作为迁移目标，统一架构、消除冲突，并为 xlsx/pptx/html 预留扩展。
...
- 新增 `docx-extractor`（lxml 直读）与 `ocr-extractor`（远程 PaddleOCR 服务 + markdown→IR，D11）。
...
| `ocr-extractor` | `specs/ocr-extractor/spec.md` | Claude | 🔄重做(D11) |
...
- **依赖**：`pydantic>=2`（核心）；optional extras：`[pdf]` PyMuPDF+pdfplumber、`[docx]` lxml、`[ocr]` httpx（远程 OCR 服务客户端，D11）、`[api]` fastapi+uvicorn、`[viz]` Pillow。
