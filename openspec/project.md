...
- **是**多格式统一组件：一套规范 IR（类型化文档树）兼容 span（可编辑 PDF）、AST（docx）与结构化 markdown（OCR 服务）。

## 2. 核心设计原则

1. **规范 IR = 类型化文档树**。它同时兼容 span（可编辑 PDF 的视觉重建产物）、AST（docx 的语义直读产物）与结构化 markdown（OCR 服务输出，见 designs/001 D11）。
2. **结构与出处分离**：内容层级源无关；bbox/页码作为节点上**可选的 `provenance`** 元数据。
3. **各格式独立 extractor，统一输出 IR**；extractor 之间**禁止横向依赖**，只能依赖 `ir-model`。
4. **span 管线是「可编辑 PDF 结构重建前端」**（OCR 改走远程服务，见 D11），不是规范 IR；其产出映射成 IR 节点。
...
| `ocr-extractor` | 重做中 | 扫描件/图片/复杂版式：远程 PaddleOCR 服务（三模型）→ markdown → IR（共享 markdown→IR 解析器） | Claude |
| `docx-extractor` | 规划中 | DOCX：lxml 直读 OpenXML → IR | Claude |
| `pipeline` | 规划中 | span 处理 Stage 引擎（**仅 pdf-extractor** 内部依赖；OCR 不再用） | 随 pdf-extractor |
