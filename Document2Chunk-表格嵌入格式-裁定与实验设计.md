...
| V4 XML | 带行/列坐标属性的规范 XML | `<row r="1"><cell c="1" colspan="2">华东</cell></row>` |
...
1. **「原始 HTML 不宜直接嵌入」在本语料+本模型上不成立。** 现状 V1 在 bge 上全指标最优或并列最优；被核查论断的第三条（业界口传+Unstructured 实践背书）在本场景被实验否定。
